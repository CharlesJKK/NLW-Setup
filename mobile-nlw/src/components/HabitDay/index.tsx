...
export default function HabitDay({amountOfHabits = 0, amountCompleted = 0, date, ...rest }: HabitDayProps){

	const amountAccomplishedPercentage = amountOfHabits > 0 ? generateProgressPercentage(amountOfHabits, amountCompleted) : 0;
	const today = dayjs().startOf("day").toDate();
	const isCurrenteDay = dayjs(date).isSame(today);
	...
	return(
		<TouchableOpacity className={clsx("rounded-lg border-2 m-1", {
			...
			["border-violet-200 border-4"]: isCurrenteDay
		})}
